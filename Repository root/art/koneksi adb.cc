...
// Pesan yang dikirim dari transportasi
...
// Pesan yang dikirim ke transportasi
...
static constexpr const char kJdwpHandshake[14] = {
    Bahasa Indonesia: 'J' , 'D' , 'W' , 'P' , '-' , 'H' , 'a' , 'n' , 'd ' , 's' , 'h' , 'a' , 'k' , 'e' };
...
void AdbConnectionState::SendDdmPacket(uint32_t id,
  ...
  *reinterpret_cast<uint32_t*>(pkt_data) = htonl(id);
  ...
  // berikutnya bendera. (0 untuk paket cmd karena DDMS).
  *(pkt_data++) = static_cast<uint8_t>(packet_type);
  switch (packet_type) {
    ...
    case DdmPacketType::kReply: {
      // Ini adalah kode kesalahan byte yang semuanya 0
      *(pkt_data++) = 0;
      *(pkt_data++) = 0;
    }
  }

  // Ini berada pada alamat yang tidak selaras jadi kita perlu melakukannya secara manual.
  // sekarang tipenya.
  ...
  Bahasa Indonesia: };
  // sekarang pkt_header memiliki header.
  // gunakan writev untuk mengirim data sebenarnya.
...
void AdbConnectionState::SendAgentFds(bool require_handshake) {
  ...
  // Gandakan fds sebelum mengirimnya.
...
bool AdbConnectionState::SetupAdbConnection() {
  ...
  const AdbConnectionClientInfo infos[] = {
      ...
      {.type = AdbConnectionClientInfoType::architecture,
       // GetInstructionSetString() mengembalikan string gaya C yang diakhiri null.
  ...
  while (!shutting_down_) {
    // Jika adbd tidak berjalan, karena debugging USB dinonaktifkan atau
    // mungkin sistem sedang me-restart untuk "adb root",
    // connect() akan gagal. Kita mengulang di sini selamanya sambil menunggunya
    // untuk kembali.
    //
    // Bangun dan melakukan polling setiap beberapa detik umumnya merupakan
    // hal buruk untuk dilakukan, tetapi kami hanya melakukan ini jika aplikasinya
    // dapat di-debug *dan* adbd tidak berjalan. Namun, demi
    // masa pakai baterai, kita harus mempertimbangkan waktu habis dan memberikan
    // muncul setelah beberapa menit jika ada yang mengirim aplikasi dengan
    // set bendera yang dapat di-debug.
    control_ctx_.reset(adbconnection_client_new(info_ptrs, std::size(infos)));
    if (control_ctx_) {
      kembalikan  benar ;
    }

    // Kami gagal terhubung.
...
void AdbConnectionState::RunPollLoop(art::Thread* self) {
  ...
  // shutting_down_ ditetapkan oleh StopDebuggerThreads
  while (!shutting_down_) {
    // Pertama, hubungkan ke adbd jika kita belum melakukannya.
    ...
    while (!shutting_down_ && control_ctx_) {
      ...
      // Secara default kami selalu tertarik pada peristiwa read dan hangup pada ctx kontrol.
      int16_t interestingControlEventSet = POLLIN | POLLRDHUP;
      if (apex_adbconnection_client_has_pending_update()) {
        // Jika kita memiliki pembaruan untuk ADBd, kita juga ingin tahu kapan kontrol ctx
        // soket dapat ditulis.
        interestingControlEventSet |= POLLOUT;
      }
      struct pollfd pollfds[4] = {
          {sleep_event_fd_, POLLIN, 0},
          // -1 sebagai fd menyebabkannya diabaikan oleh polling
...
bool ValidateJdwpOptions(const std::string& opts) {
  bool res = true;
  // Plugin adbconnection mengharuskan agen jdwp dikonfigurasi sebagai 'server' karena itu
  // adalah apa yang diharapkan adb dan jika tidak kita akan menemui jalan buntu karena utas loop polling berhenti menunggu
  // agar fd dapat diwariskan.
  if (ContainsArgument(opts, "server=n")) {
    res = false;
    LOG(ERROR) << "Cannot start jdwp debugging with server=n from adbconnection.";
  }
  // Kami tidak memulai agen jdwp sampai thread sudah berjalan. Sudah terlambat untuk menundanya
  // semuanya.
...
# jika didefinisikan(__ANDROID__)
batalkan FixLogfile(JdwpArgs& parameter) {
  const std::string kLogfile = "berkas log" ;
  // Di Android, aplikasi tidak akan memiliki akses tulis ke cwd (yang merupakan "/").
  // Jika jalur relatif diberikan, kita perlu menambalnya dengan jalur yang dapat ditulis
  // lokasi. Untuk saat ini, kami menggunakan /data/data/<NAMA_PKG>.
  // Perhatikan bahwa /data/local/tmp/ juga dipertimbangkan tetapi itu bukan kandidat yang baik karena aplikasi tidak
  // memiliki akses tulis ke sana.

  jika (!parameter.berisi(kLogfile)) {
    kembali ;
  }

  std::string& logfile = parameter.dapatkan(kLogfile);
  jika (logfile.front() == '/' ) {
    // Kami hanya memperbaiki file log jika tidak menggunakan jalur absolut
    kembali ;
  }

  std::string namaPaket = art::Runtime::Current()->GetProcessPackageName();
  jika (namapaket.kosong()) {
    VLOG(jdwp) << "Tidak dapat memperbaiki jalur relatif logfile='" + logfile + "' tanpa nama paket." ;
    kembali ;
  }
  parameter.put(kLogfile, "/data/data/" + namapaket + "/" + logfile);
}
# kalau tidak
...
std::string AdbConnectionState::MakeAgentArg() {
  ...
  // Lihat komentar di atas untuk mengetahui mengapa kita perlu menggunakan server=y. Karena agen menggunakan server=n sebagai default
  // kita harus selalu mengaturnya.
  parameters.put("server", "y");

  // Lihat komentar di atas untuk mengetahui mengapa kita perlu suspend=n. Karena agen defaultnya adalah
  // suspend=y kita harus selalu mengaturnya.
  ...
  // Jika logfile relatif, kita perlu memperbaikinya.
  FixLogfile(parameters);

  // TODO Dapatkan agent_name_ dari sesuatu yang dapat ditetapkan pengguna?
...
void AdbConnectionState::StopDebuggerThreads() {
  // Sistem agen reguler akan mengurus pembongkaran agen (jika diperlukan).
  shutting_down_ = true;
  WakeupPollLoop();
}
...
extern "C" bool ArtPlugin_Initialize() {
  ...
  // TODO Sediakan beberapa cara bagi aplikasi untuk mengatur ini mungkin?
  gState.emplace(kDefaultJdwpAgentName);
  return ValidateJdwpOptions(art::Runtime::Current()->GetJdwpOptions());
}

extern "C" bool ArtPlugin_Deinitialize() {
  // Kita sebenarnya tidak perlu melakukan apa pun di sini. Debugger (jika ada)
  // terlampir) dimatikan dengan pindah ke fase runtime kDeath dan
  // thread adbconnection dimatikan oleh StopDebugger.
  kembalikan  benar ;
}
